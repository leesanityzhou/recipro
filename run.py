...
from recipro.config import AppConfig, load_settings
...
def _pick_ambient() -> tuple[str | None, str | None]:
    from recipro.ambient import PROVIDERS, available_providers
...
def main() -> int:
    from recipro.ambient import PROVIDERS, available_providers, init_agent
    ...
    if need_setup:
        ...
        _save_prefs(prefs)
        print(f"\n  Preferences saved. Run --reconfigure to change later.")
    ...
    # Load base settings from config.yaml
    settings = load_settings(PROJECT_ROOT / "config.yaml")

    # Build config: config.yaml (toggles) + preferences (backends/models) + CLI (flags)
    config = AppConfig(
